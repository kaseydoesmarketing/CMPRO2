...
async function runOnce(iter) {
  ...
  if (MODE === 'template') {
    // 4/5) Parse JSON + AJV validate via server-side schema (reuse for smoke)
    ...
    // 6) ZIP sanity: ensure size reasonable when images present
...
(async () => {
  ...
  while (pass < 2 && attempts < 4) {
    ...
    try {
      ...
      pass = 0; // require two consecutive passes
